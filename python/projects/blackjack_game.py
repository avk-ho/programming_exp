...
# notes
# goal for the player is having a hand with a value superior to the dealer's but < 21
# J Q K = 10
# A = 1 or 11 (whichever the most useful)

# each round, player bets min 1$, max their reserve
# both the player and dealer receive 2 cards, face up for the player, 1 face up for the dealer
# player can then choose to hit or stay, hit +1 card, stay stop with their current cards
# once the player stays, the dealer reveals their second card
# if dealer's hand >= 17, they must stay
# if < 17, they must hit until >= 17
# dealer also loses if hand > 21

# special case, "natural"
# if the player has a 21, they automatically win if the dealer doesn't have 21 too
# they receive their bet back + 1.5 their bet (bet * 2.5)

# when both have stayed, whoever has the greater value wins
# if player wins, they receive the double their bet
# if dealer wins, they take the player's bet
# if it's a tie, the player receives their bet back

# the player will start with 500$
# game is over if the player has 0$ or decides not to play a hand

# 52 cards deck (4 suits, 13 cards each)
# suits are ♦ diamond, ♥ heart, ♣ club, ♠ spade
# cards are 2-9, K, Q, J, T, A